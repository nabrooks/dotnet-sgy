...
{
    public static class IEnumerableConstructors
    {
        public static IEnumerable<T> Range<T>(int iterationCount, Func<int, T> iterationDeltaFunction)
        {
            T[] result = new T[iterationCount];
            for (int i = 0; i < iterationCount; i++)
            {
                result[i] = iterationDeltaFunction(i);
            }
            return result;
        }
    }
